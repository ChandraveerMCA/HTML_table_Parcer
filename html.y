...
BODY1    : STD DATA ETD // Rule that matches a table cell with standard alignment and data
         ...
         | BODY1 STD DATA ETD // Rule that matches multiple table cells with standard alignment and data
         | BODY1 STH DATA ETH // Rule that matches multiple table cells with header alignment and data
         ;

DATA    : DIGIT   // Rule that matches a numeric data item
        | ALNUM   // Rule that matches an alphanumeric data item
        | DATA STAG // Rule that matches data followed by a subtable definition
